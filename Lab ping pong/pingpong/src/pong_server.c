...
/*** TO BE DONE START ***/
	...
	freeaddrinfo(pong_addrinfo); // since it will never be used again, it can be closed

	if(bind_rv == 0) {
		*pong_port = port_number; // passo il valore a pong_port cosicchè la variabile venga inizializzata correttamente nel serve_client().		
		return udp_socket;
	} 
	/*** else ***/
