...
/*** TO BE DONE START ***/
	if(clock_gettime(CLOCK_TYPE, &send_time))
		fail_errno("Error in saving current time");

/*** TO BE DONE END ***/

    /*** Send the message through the socket ***/
/*** TO BE DONE START ***/
	sent_bytes = 0;	// initialization as good programming technique
	
	sent_bytes = blocking_write_all(tcp_socket, message, msg_size); 
	// Se i byte effettivamente inviati differiscono da quelli 'richiesti di essere inviati' fallisce, questo perchè per come è definita la funzione, 
		// l'iterazione è già svolta all'interno. Eventuali valori minori di zero o altri tipi di errori vengono intercettati dalla funzione assert() 
		// presente all'interno dell'implementazione della blocking_write_all stessa
...
/*** TO BE DONE START ***/
	...
	if(tcp_socket == -1) 
		fail_errno("Could not open a socket");
	if(connect(tcp_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) == -1) 
		fail_errno("Could not connect to the specified address");
...
/*** TO BE DONE START ***/
// Non si sente la necessità di una write_all (readwrite.c) per il messaggio-richiesta (si pensa che la richiesta venga inviata integralmente al server 
		// e non solo parzialmente, tale da dover necessariamente iterare)
