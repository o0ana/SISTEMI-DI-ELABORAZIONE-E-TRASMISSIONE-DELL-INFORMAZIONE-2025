...
/*** TO BE DONE START ***/
sprintf(message, "%d\n", msg_no);//copia i numeri di byte dalla memoria puntata nella memoria destinataria.
//il messaggio viene messo nella temporaneamente nella memoria, posizionato all'inizio del messaggio
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
clock_gettime(CLOCK_TYPE, &send_time); //mette il tempo corrente quando manda il messaggio
...
/*** TO BE DONE START ***/
clock_gettime(CLOCK_TYPE, &recv_time);  //mette il tempo corrente quando si riceve il messaggio
/*** TO BE DONE END ***/
...
int prepare_udp_socket(char *pong_addr, char *pong_port)
{
	...
	memset(&gai_hints, 0, sizeof gai_hints);//imposta il conteggio byte di gai_hints al valore di sizeof gai_hints
/*** TO BE DONE START ***/
	//gli hints vengono chiamati così  proprio perchè sono suggerimenti che definiscono il socket
	gai_hints.ai_family = AF_INET;  // indirizzi IPv4 
	gai_hints.ai_socktype = SOCK_DGRAM;  // Datagram socket type (UDP), supporta il flusso bidirezionale dei dati
	gai_hints.ai_protocol = IPPROTO_UDP;  // usa il protocollo UDP 
...
/*** TO BE DONE END ***/
	//  Utilizzare la system call socket() e usa la funzione fail_errno() ( è definita nel file function.c ) in caso di errore
...
/*** TO BE DONE START ***/
	...
	//fcntl esegue operazioni su ping_socket, in questo caso l'operazione è F_GETFL cioè imposta i flags del file descriptor al valore specificato da arg che è O_NONBLOCK
/*** TO BE DONE END ***/

    /*** call getaddrinfo() in order to get Pong Server address in binary form ***/
/*** TO BE DONE START ***/
 	gai_rv = getaddrinfo(pong_addr, pong_port, &gai_hints, &pong_addrinfo); //?
	//getaddrinfo è una funzione che fornisce una conversione da nome a un indirizzo.
    if (gai_rv != 0) {
        fail("getaddrinfo failed");
    }
...
if (connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1){
		fail_errno("UDP Ping non è collegato con il Pong server\n");
}
//connect stabilisce una connessione a un socket specificato.
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
    ...
    gai_hints.ai_socktype = SOCK_STREAM;   // TCP socket
    gai_hints.ai_protocol = IPPROTO_TCP;   // protocollo TCP 
...
/*** TO BE DONE START ***/
	if((ask_socket= socket(gai_hints.ai_family, gai_hints.ai_socktype,gai_hints.ai_protocol))==-1){
		fail_errno("UDP Ping non è connesso\n");
	}
	if (connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen)== -1) fail_errno ("connect error\n");    
...
/** TO BE DONE START ***/
	...
	/*if(write(ask_socket, request, strlen(request))>msg_size){ //strlen(request) Restituisce la lunghezza di una stringa
		fail_errno("4\n");
	}*/
...
/*** TO BE DONE START ***/
 	if(strncmp(answer,"OK", 2) != 0) // comparazione fra stringhe
		fail_errno("ERROR\n");
