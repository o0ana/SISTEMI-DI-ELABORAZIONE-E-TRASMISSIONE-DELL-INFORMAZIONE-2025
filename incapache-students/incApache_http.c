...
/*** TO BE DONE 8.0 START ***/
	//creazione id per un nuovo utente
...
int keep_track_of_UID(int myUID)
{
     ...
     *** Be careful in order to avoid race conditions ***/
/*** TO BE DONE 8.0 START ***/
	//utente che si conette un altra volta
	pthread_mutex_lock(&cookie_mutex);
	++UserTracker[myUID];
...
/*** TO BE DONE 8.0 START ***/
	if(gmtime_r(&now_t, &now_tm) == NULL) fail_errno("fail gmtime_r ");
/*** TO BE DONE 8.0 END ***/
...
/*** TO BE DONE 8.0 START ***/
		char cookie_as_string[3];
		sprintf(cookie_as_string, "%d", cookie);
		strcat(http_header, "\r\nSet Cookie UserID=");
		strcat(http_header, cookie_as_string);
...
/*** TO BE DONE 8.0 START ***/
		if(gmtime_r(&file_modification_time, &file_modification_tm) == NULL) fail_errno("gmtime fail");
...
/*** TO BE DONE 8.0 START ***/
		...
		while(sent_bytes<file_size){
			sent_now = sendfile(client_fd, fd, &sent_bytes, file_size-sent_bytes);
			if(sent_now == -1) fail_errno("sendfile error");
			sent_bytes += sent_now;
		}
		close(fd);
...
/*** TO BE DONE 8.0 START ***/
					char* cookie_name = strtok_r(NULL, "=", &strtokr_save);
...
/*** TO BE DONE 8.0 START ***/
					...
					{	//solo se ci sono state modifiche e la data è valida
						option_val = strtok_r(NULL, "\r\n", &strtokr_save);
						if(strftime(option_val, MAX_TIME_STR, "%a, %d %b %Y %T GMT", &since_tm))
...
/*** TO BE OPTIONALLY CHANGED END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 8.0 START ***/
				time_t since_time = my_timegm(&since_tm); //funzione che va bene con i thread
				time_t file_modification_time = stat_p -> st_mtime;
				if(file_modification_time>since_time) http_method = METHOD_GET;
