...
/*** TO BE DONE 8.1 START ***/
	...
	//Funzione: Gestisce il join di tutti i thread di risposta associati a una determinata connessione.
	if(!to_join[conn_no]) return; // riferimento all'ultimo response thread inserito
	i = to_join[conn_no] - thread_ids;  // riferimento al response thread precedente

	if(pthread_join(thread_ids[i], NULL)) fail_errno("Error in join_all_threads(): pthread_join() failed");

	if(pthread_mutex_lock(&threads_mutex)) fail_errno("Could not acquire lock"); // lock effettuato in fase di scrittura
	
	if(--(no_response_threads[conn_no]) > 0)
		no_free_threads++;
	connection_no[i] = FREE_SLOT;
	
	to_join[conn_no] = NULL; // riporto gli ultimi thread a NULL, come all'inizio

	if(pthread_mutex_unlock(&threads_mutex)) fail_errno("Could not unlock thread");
...
/*** TO BE DONE 8.1 START ***/
	...
	//Funzione: Effettua il join del thread precedente associato a un dato thread attuale.
	if(!to_join[thrd_no]) return;
	i = to_join[thrd_no] - thread_ids; // otteniamo l'indice previous del response thread rispetto al corrente
	
	if(pthread_join(thread_ids[i], NULL)) fail_errno("Error in join_prev_thread(): pthread_join() failed");

	if(pthread_mutex_lock(&threads_mutex)) fail_errno("Could not acquire lock"); // lock effettuato in fase di scrittura
	
	conn_no = connection_no[thrd_no]; // contenente l'indice della connessione
	
	if(--(no_response_threads[conn_no])>0) // per tenere sempre almeno 1 response thread, decremento controllando quanti response thread di risposta ho ancora
		no_free_threads++;
	connection_no[i] = FREE_SLOT;
...
/*** TO BE DONE 8.1 END ***/
	...
	//Funzione: È il thread di risposta che invia la risposta al client e libera le risorse.
...
/*** TO BE DONE 8.1 START ***/
	//Gestisce le connessioni dei client e inizializza la struttura to_join.
...
/*** TO BE DONE 8.0 START ***/
	//Determina il MIME type di un file evitando race conditions.
...
/*** TO BE DONE 8.1 START ***/
	//Funzione: Crea un nuovo thread di risposta e aggiorna la struttura to_join.
	to_join[new_thread_idx] = to_join[connection_idx];
	
	// Aggiorniamo in connection_idx il nuovo-ultimo response thread inserito
